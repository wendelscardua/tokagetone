...
namespace GGSound {
...
#ifdef FEATURE_DPCM
  extern "C" void *_ggsound_base_address_dpcm_sample_table;
  extern "C" void *_ggsound_base_address_dpcm_note_to_sample_index;
  extern "C" void *_ggsound_base_address_dpcm_note_to_sample_length;
  extern "C" void *_ggsound_base_address_dpcm_note_to_loop_pitch_index;
  extern "C" DPCM_STATE _ggsound_apu_dpcm_state;
#endif
...
#ifdef FEATURE_DPCM
    _ggsound_base_address_dpcm_sample_table = dpcm_pointers[0];
    _ggsound_base_address_dpcm_note_to_sample_index = dpcm_pointers[1];
    _ggsound_base_address_dpcm_note_to_sample_length = dpcm_pointers[2];
    _ggsound_base_address_dpcm_note_to_loop_pitch_index = dpcm_pointers[3];
#endif
...
#ifdef FEATURE_DPCM
    _ggsound_apu_dpcm_state = DPCM_STATE::NOP;
#endif
...
#ifdef FEATURE_DPCM
    stream_stop(Stream::DPCM);
    if (track.dpcm_stream != 0) {
      stream_initialize(Stream::DPCM, Channel::DPCM, track.dpcm_stream);
      _ggsound_apu_dpcm_state = DPCM_STATE::NOP;
      // Reset load counter to safeguard against accumulating too far in one
      // direction
      POKE(0x4011, 0);

      _ggsound_stream_tempo_lo[(u8)Stream::DPCM] =
          _ggsound_stream_tempo_counter_lo[(u8)Stream::DPCM] = (u8)track_tempo;
      _ggsound_stream_tempo_hi[(u8)Stream::DPCM] =
          _ggsound_stream_tempo_counter_hi[(u8)Stream::DPCM] =
              (u8)(track_tempo >> 8);
    }
#endif
  ...
  __attribute__((noinline)) void play_sfx(SFX sfx, SFXPriority priority) {
    ...
    // Load dpcm stream.
    if (track.dpcm_stream != 0) {
      stream_initialize((Stream)priority, Channel::DPCM, track.dpcm_stream);
      _ggsound_stream_tempo_lo[(u8)priority] =
          _ggsound_stream_tempo_counter_lo[(u8)priority] = (u8)track_tempo;
      _ggsound_stream_tempo_lo[(u8)priority] =
          _ggsound_stream_tempo_counter_lo[(u8)priority] = (u8)track_tempo;
